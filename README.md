...
> Важно: сервис не показывает и не импортирует ФИО, адреса, телефоны, документы или другие персональные данные. Отсутствие записи не доказывает отсутствие автомобиля, регистрации, розыска, страхования или ДТП.
...
.
├── alembic/
│   ├── versions/{20260819_0001_initial.py,20260820_0002_vehicle_external_history.py}
│   └── env.py
├── app/
...
Cloudflare-версия сервиса вычисляет первую/последнюю известную регистрацию, признак импорта
по первой открытой операции, предполагаемые смены владельца, периоды владения, смены номеров,
регионы, изменения цвета/топлива/двигателя/кузова/назначения/массы и быстрые перепродажи.
VIN-блок локально показывает WMI, регион, известного производителя/страну WMI, модельный год,
VIS и структурную проверку. Контрольная цифра применяется только к VIN регионов, где такая
проверка нормативно ожидается; европейский VIN не объявляется поддельным из-за check digit.

Индекс истории — собственная эвристическая оценка сервиса, а не официальный рейтинг,
экспертиза или юридический вывод. Факты регистрации и расчётные признаки в отчёте разделены.

### Проверка розыска

Используется официальный набор Национальной полиции Украины «Інформація про транспортні
засоби, що перебувають у розшуку у зв’язку з їх незаконним заволодінням»:

<https://data.gov.ua/dataset/ac1a3a9d-512b-446b-9b0c-1383d38ce474>

GitHub Actions проверяет новую версию каждые шесть часов и независимо публикует компактный
индекс номера/VIN. Отчёт формулирует отрицательный результат только как «совпадений в открытом
реестре не найдено» и показывает дату версии. Если источник недоступен, результат отмечается
как недоступный, а не как отсутствие розыска.

Персональная проверка ДТП по VIN/номеру не заявляется: открытая статистика ДТП не является
реестром, достоверно связывающим конкретный автомобиль с конкретным происшествием.
